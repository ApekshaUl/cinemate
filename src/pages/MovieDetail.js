...
export const MovieDetail = ({title}) => {
  ...
  useEffect(()=>
  {
    async function fetchMovies(){
        const respone = await fetch(`https://api.themoviedb.org/3/movie/${params.id}?api_key=f1e65a939d2b9a61ff79351420ba0a78`);
          ...
          <p className="my-4">{data.overview}</p>
            ...
            {data.genres ? (
              ...
              {data.genres.map((genre) => (
                  <span className="mr-2 border p-2 rounded border-gray-900 dark:border-gray-600" key = {genre.id}>{genre.name}</span>
              ))}  
            </p>
            ): ""}
            ...
            <p className="my-4">
              ...
              <a className="hover:cursor-pointer underline"href={`https://www.imdb.com/title/${data.imdb_id}`} target="_blank" rel="noreferrer">{data.imdb_id}</a>
            </p>
